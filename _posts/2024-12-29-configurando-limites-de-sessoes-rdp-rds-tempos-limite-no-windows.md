...
Quando um usuário fecha a janela de sessão **RDP/RDS** em um cliente de terminal <kbd>(mstsc.exe, RDCMan ou cliente web HTML5 do Remote Desktop)</kbd> simplesmente clicando na cruz no canto superior direito sem fazer logoff,
...
Sessões RDP desconectadas geralmente causam um problema de bloqueio de conta de usuário de domínio após uma alteração de senha (quando a sessão RDS continua a ser executada sob a senha antiga do usuário).
